...
- Name: tests\usuario.favoritos.spec.js >> Acceder a favoritos del usuario
- Location: tests\usuario.favoritos.spec.js:4:1
...
```ts
  ...
  3  | 
  4  | test('Acceder a favoritos del usuario', async ({ page }) => {
  5  | 
...
> 16 |     await expect(page.locator('[data-test="nav-menu"]')).toBeVisible();
  ...
  19 | 
  20 |     await page.locator('[data-test="nav-favorites"]').click();
  21 | 
  22 |     await expect(page.locator('body'))
  23 |         .toContainText('Favorites');
  24 | 
  25 | });
```