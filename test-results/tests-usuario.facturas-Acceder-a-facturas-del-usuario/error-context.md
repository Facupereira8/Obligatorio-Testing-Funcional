...
- Name: tests\usuario.facturas.spec.js >> Acceder a facturas del usuario
- Location: tests\usuario.facturas.spec.js:4:1
...
```ts
  ...
  3  | 
  4  | test('Acceder a facturas del usuario', async ({ page }) => {
  5  | 
...
> 16 |     await expect(page.locator('[data-test="nav-menu"]')).toBeVisible();
  ...
  19 | 
  20 |     await page.locator('[data-test="nav-invoices"]').click();
  21 | 
  22 |     await expect(page.locator('body'))
  23 |         .toContainText('Invoices');
  24 | 
  25 | });
```