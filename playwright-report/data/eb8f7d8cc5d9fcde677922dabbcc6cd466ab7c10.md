...
- Name: tests\usuario.perfil.spec.js >> Acceder al perfil de usuario
- Location: tests\usuario.perfil.spec.js:4:1
...
```ts
  ...
  3  | 
  4  | test('Acceder al perfil de usuario', async ({ page }) => {
  5  | 
...
> 16 |     await expect(page.locator('[data-test="nav-menu"]')).toBeVisible();
  ...
  19 | 
  20 |     await page.locator('[data-test="nav-profile"]').click();
  21 | 
  22 |     await expect(page.locator('body'))
  23 |         .toContainText('My profile');
  24 | 
  25 | });
```